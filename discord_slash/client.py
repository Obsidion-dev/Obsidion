...
class SlashCommand:
    ...
    def __init__(
        ...
        if (
            ...
            if not hasattr(self._discord, "slash"):
                ...
                raise error.DuplicateSlashClient(
                    "You can't have duplicate SlashCommand instances!"
                )
    ...
    async def to_dict(self):
        ...
        wait = (
            {}
        )  # Before merging to return dict, let's first put commands to temporary dict.
        for x in self.commands:
    ...
    async def sync_all_commands(self, delete_from_unused_guilds=False):
        ...
        for x in cmds["guild"]:
            await self.req.put_slash_commands(
                slash_commands=cmds["guild"][x], guild_id=x
            )
    ...
    def add_subcommand(
        ...
        base = base.lower()
        subcommand_group = (
            subcommand_group.lower() if subcommand_group else subcommand_group
        )
        name = name or cmd.__name__
        name = name.lower()
        description = description or getdoc(cmd)
