...
class Minecraft(commands.Cog):
    ...
    async def tick2second(self, ctx: commands.Context, ticks: int) -> None:
        ...
        seconds = ticks / 20
        await ctx.send(_("It takes {seconds} second for {ticks} to happen.").format(seconds=seconds, ticks=ticks))

    @commands.command()
    async def second2tick(self, ctx: commands.Context, seconds: float) -> None:
        """Convert ticks to seconds."""
        ticks = seconds * 20
        await ctx.send(_("There are {ticks} ticks in {seconds} seconds").format(ticks=ticks, seconds=seconds))
